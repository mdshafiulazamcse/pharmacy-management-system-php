...
function getProducts($categories = [], $brands = [], $minPrice = null, $maxPrice = null, $minRating = null, $sort = null) {
    ...
    $sql = "SELECT * FROM supplements WHERE 1=1";
    $params = [];
    $types = "";
    ...
    switch ($sort) {
        ...
        case 'newest':
            ...
            break;
        default:
            $sql .= " ORDER BY rating_count DESC"; // Default to featured
    }
...
function getCategories() {
    global $conn;
    $sql = "SELECT DISTINCT category FROM supplements";
...
function getBrands() {
    global $conn;
    $sql = "SELECT DISTINCT brand FROM supplements";
    $result = $conn->query($sql);
    return $result->fetch_all(MYSQLI_ASSOC);
}
?>
